...
contract TestRaidswapV3Router is IRaidswapV3SwapCallback {
    ...
    function swapForExact0Multi(
        ...
        pools[0] = poolInput;
        IRaidswapV3Pool(poolOutput).swap(
            recipient,
            false,
            -amount0Out.toInt256(),
    ...
    function RaidswapV3SwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid =
                amount0Delta > 0 ? IRaidswapV3Pool(msg.sender).token0() : IRaidswapV3Pool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == IRaidswapV3Pool(pools[0]).token1();
            IRaidswapV3Pool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(IRaidswapV3Pool(msg.sender).token0()).transferFrom(
                    payer,
                    msg.sender,
                    uint256(amount0Delta)
                );
            } else {
                IERC20Minimal(IRaidswapV3Pool(msg.sender).token1()).transferFrom(
                    payer,
                    msg.sender,
