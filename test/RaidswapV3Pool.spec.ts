...
import { TestRaidswapV3SwapPay } from '../typechain/TestRaidswapV3SwapPay'
...
import { TestRaidswapV3ReentrantCallee } from '../typechain/TestRaidswapV3ReentrantCallee'
...
describe('RaidswapV3Pool', () => {
  ...
  let pool: MockTimeRaidswapV3Pool

  let swapTarget: TestRaidswapV3Callee
  ...
  async function initializeAtZeroTick(pool: MockTimeRaidswapV3Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  // https://github.com/Raidswap/uniswap-v3-core/issues/214
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestRaidswapV3ReentrantCallee')
      ).deploy()) as TestRaidswapV3ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestRaidswapV3SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestRaidswapV3SwapPay')
      underpay = (await underpayFactory.deploy()) as TestRaidswapV3SwapPay
